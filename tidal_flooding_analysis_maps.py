...
def make_directories(location):
    all_maps_dir = 'C:/Users/kristydahl/Desktop/GIS_data/miltary_bases'
...
def set_file_paths(location, dem_file, slr_file):
    ...
    results_path = os.path.join(location_path, "results")
...
def water_surfaces(location, dem_file, slr_file):
    ...
    slr_data = genfromtxt(slr_csv_data,skip_header=1,delimiter=',')
    dem = paths['dem']
    ...
    for row in slr_data:
        ...
        print(year)
        print(value)
        surface = Con(Raster(dem) <=row[1], 1) #Use DEM and SLR data that's still in meters!
        outname = (paths['results'] + str("/" + 'tf_extent' + "_" + str(int(row[0]))+ '.tif'))
...
def region_group_extract(location, dem_file,slr_file):
    ...
    for surface in surfaces:
        ...
        filename = os.path.basename(fullname)
        outname_polygon = (paths['results'] + "/" + str('final_polygon_' + filename.replace('tif','shp')))
...
def create_maps(location,dem_file,slr_file):
    ...
    testmap = os.path.join(location_path, 'tf_testmap.mxd')
    sym_layer = os.path.join(sym_layers_path,'water_style.lyr')
    print(sym_layer)

    #shps = region_group_extract(location, dem_file, slr_file)
    shps = glob.glob1(results_path,'final_polygon*.shp')

    mxd = arcpy.mapping.MapDocument(testmap)
    df = arcpy.mapping.ListDataFrames(mxd,"Layers")[0]

    outmap = os.path.join(map_path, location + '_tidal_flooding_surfaces.mxd')
